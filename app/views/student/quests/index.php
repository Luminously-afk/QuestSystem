...
<div class="mb-8 font-mono text-xs font-bold uppercase flex items-center gap-2 text-secondary">
    ...
    <span class="text-black">ACTIVE QUESTS</span>
</div>

<div class="flex justify-between items-center mb-8">
    <h2 class="font-h1 text-h1 uppercase text-on-background">ACTIVE QUESTS</h2>
</div>
...
<section class="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
    <div class="p-4 border-b-4 border-black flex flex-wrap gap-4 justify-between items-center bg-zinc-50">
        <h2 class="font-h2 text-black uppercase font-black">AVAILABLE MISSIONS</h2>
    </div>
    <div class="overflow-x-auto">
        <?php if (empty($quests)): ?>
            <div class="p-6 font-mono text-sm uppercase">No active quests at the moment.</div>
        <?php else: ?>
...
<dialog id="submit-quest-modal" class="bg-white border-4 border-black p-0 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] max-w-lg w-full backdrop:bg-black/60">
    ...
    <form method="post" action="" id="submit-form" class="p-6 flex flex-col gap-4 font-mono">
        ...
        <div class="flex flex-col gap-2">
            ...
            <textarea name="proof_text" id="submit-proof-text" class="border-2 border-black p-2 focus:outline-none focus:border-primary" rows="5" required><?php echo htmlspecialchars($proof_text ?? ''); ?></textarea>
        </div>
...
<script>
    ...
    <?php if (!empty($open_submit_modal)): ?>
        window.addEventListener('DOMContentLoaded', () => { 
            // In case of error, the user has the quest ID.
            // Ideally we'd need to find the quest object to pass to the function, but since they failed submission,
            // we could either just open the modal and rely on backend for validation, or find the quest in JS.
