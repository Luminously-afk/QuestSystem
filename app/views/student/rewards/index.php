...
<div class="flex justify-between items-center mb-8">
    ...
    <div class="bg-black text-[#FFD54F] border-4 border-black px-4 py-2 font-mono font-bold text-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center gap-2">
...
<section class="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
    <div class="p-4 border-b-4 border-black flex flex-wrap gap-4 justify-between items-center bg-zinc-50">
        <h2 class="font-h2 text-black uppercase font-black">AVAILABLE REWARDS</h2>
    </div>
                        ...
                        <tr class="border-b-2 border-zinc-100 hover:bg-zinc-50">
                            ...
                            <td class="p-4">
                                <?php if ($redemptionStatus === 'approved'): ?>
                                    <span class="px-2 py-1 text-[10px] border border-black font-black uppercase bg-[#9aed83] text-[#1e6d12]">APPROVED</span>
                                <?php elseif ($redemptionStatus === 'pending'): ?>
                                    <span class="px-2 py-1 text-[10px] border border-black font-black uppercase bg-[#ffd54f] text-black">PENDING</span>
                                <?php elseif ($redemptionStatus === 'rejected'): ?>
                                    <span class="px-2 py-1 text-[10px] border border-black font-black uppercase bg-[#ffdad6] text-[#ba1a1a]">REJECTED</span>
                                <?php else: ?>
                                    <span class="px-2 py-1 text-[10px] border border-black font-black uppercase bg-zinc-200 text-zinc-600">AVAILABLE</span>
                                <?php endif; ?>
                            </td>
                            <td class="p-4 text-right">
                                <?php if ($canRequest): ?>
                                    <button onclick="openRequestModal(<?php echo htmlspecialchars(json_encode($reward)); ?>)" class="bg-tertiary text-white border-2 border-black px-3 py-1 font-button-text hover:bg-[#154d0d] uppercase">
                                        REQUEST
                                    </button>
                                <?php elseif ($redemptionStatus === 'pending'): ?>
                                    <span class="text-[10px] text-zinc-400 font-bold uppercase">REQUESTED</span>
                                ...
                                <?php elseif ($user_points < (int) $reward['required_points']): ?>
                                    <span class="text-[10px] text-zinc-400 font-bold uppercase">NOT ENOUGH XP</span>
                                <?php else: ?>
                                    <span class="text-[10px] text-zinc-400 font-bold uppercase">UNAVAILABLE</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<!-- Request Dialog -->
<dialog id="request-reward-modal" class="bg-white border-4 border-black p-0 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] max-w-sm w-full backdrop:bg-black/60">
    ...
    <form method="post" action="" id="request-form" class="p-6 flex flex-col gap-4 font-mono">
        ...
        <div class="flex gap-4 mt-4">
            <button type="button" onclick="closeModal('request-reward-modal')" class="flex-1 bg-zinc-200 border-2 border-black p-3 font-button-text hover:bg-zinc-300">CANCEL</button>
            <button type="submit" class="flex-1 bg-tertiary text-white border-2 border-black p-3 font-button-text hover:bg-[#154d0d]">CONFIRM</button>
        </div>
    </form>
</dialog>
