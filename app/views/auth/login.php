...
<div class="flex justify-center items-center min-h-[70vh]">
    <div class="w-full max-w-md bg-surface-container-lowest border-2 border-on-surface shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] flex flex-col">
        <div class="bg-primary border-b-4 border-on-surface p-4 text-center">
            <h2 class="font-h1 text-h2 uppercase text-white tracking-widest" style="text-shadow: 2px 2px 0px #000;">LOGIN</h2>
        </div>

        <div class="p-8">
            <?php if (!empty($success)): ?>
                <div class="bg-[#9aed83] border-2 border-on-surface p-4 mb-6 font-mono font-bold uppercase text-sm pixel-shadow-sm text-on-surface">
            ...
            <form method="post" action="<?php echo BASE_URL; ?>/auth/login" class="flex flex-col gap-6 font-mono">
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="identifier" class="border-2 border-on-surface p-3 focus:outline-none focus:border-primary text-lg" value="<?php echo htmlspecialchars($old['identifier'] ?? ''); ?>" required autofocus>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="password" name="password" class="border-2 border-on-surface p-3 focus:outline-none focus:border-primary text-lg" required>
                </div>
                
                <button type="submit" class="bg-primary text-white border-2 border-on-surface p-4 font-button-text hover:bg-on-primary-fixed-variant active:translate-y-1 pixel-shadow-sm uppercase text-lg mt-2">
                    START <span class="material-symbols-outlined align-middle ml-2 text-xl">play_arrow</span>
                </button>
            </form>

            <div class="mt-8 pt-6 border-t-4 border-on-surface border-dashed text-center">
